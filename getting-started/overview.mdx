...
<Tabs>
  <Tab title="Cloud Platform">
    ...
    - **Gallery storage** and synchronization of face templates to FMU devices via RabbitMQ
  ...
  <Tab title="Edge Devices (FMU)">
    Facial Matching Units (FMUs) are deployed on-premises at each monitored location. Each FMU runs a full instance of the **Cognetic FaceVACS VideoScan** engine — the commercial biometric platform that powers all facial recognition in IVT.

    - **Local facial detection** — FaceVACS extracts faces from camera streams in real time via the Camera Capture Service (CCS)
    - **Template generation** — the Cognetic engine converts detected faces into mathematical biometric embeddings
    - **Gallery matching** — FaceVACS performs 1:N matching against locally stored gallery data via its SOAP API
    - **Result synchronization** — only detection metadata and thumbnails are pushed to the cloud; raw video stays local
    - **Offline resilience** — FMUs continue operating when cloud connectivity is interrupted, queuing results for later sync
    - **Per-company isolation** — each FMU is assigned to a single company and stores only that company's biometric data

    FMUs reduce bandwidth requirements and ensure sub-second recognition latency. For a deep dive into the internal architecture, see [How It Works](/getting-started/how-it-works).
  </Tab>
...
<CardGroup cols={2}>
  ...
  <Card title="How It Works" icon="gears" href="/getting-started/how-it-works">
    Learn how the Cognetic engine, FMU devices, and cloud platform work together.
  </Card>
</CardGroup>
